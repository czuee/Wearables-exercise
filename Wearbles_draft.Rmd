...
- Some of the variables hav high skewness & kurtosis
...
nzv[nzv$zeroVar,]
nzv_del <- rownames(nzv[nzv$zeroVar,])
...
M <- abs(cor(trainproc[ ,-c(1,119)]))
#corrplot(M, method = "color", diag = FALSE, addrect = TRUE, tl.pos = FALSE)
diag(M) <- 0
...
par(mfrow=c(1,2));hist(trainproc$var_yaw_belt);qqnorm(trainproc$roll_belt)
```
